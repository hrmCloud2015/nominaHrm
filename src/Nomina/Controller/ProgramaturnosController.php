...
class ProgramaturnosController extends AbstractActionController
{
    public function indexAction()
    {
        return new ViewModel();
    }
    private $lin  = "/nomina/programaturnos/list"; // Variable lin de acceso  0 (C)
        ...
        if($this->getRequest()->isPost()) // Actulizar datos
        {
           ...
           if ($request->isPost()) 
           {
              ...
              try 
              {
                ...
                if ($data->tipo==1)// Insertar datos para turnos  
                {   
                    $guardado = 1;
                    $datP = $d->getProgramaPeriodo();
                    $empArreglo = str_replace('emp','',$data->empArreglo);
                    $datosE = $d->getGeneral("select id 
                                                from a_empleados
                                                   where id in (".$empArreglo.")");              
                    foreach ($datosE as $datE) 
                    {
                       $idEmp = $datE['id'];                                                         

                       // Borrar datos anteriroes 
                       $dat = $d->getGeneral1("select id, count(id) as num  
                                                from n_nov_prog where idEmp=".$idEmp);
                       if ($dat['num']>0)
                       {                              
                          $d->modGeneral("delete from n_nov_prog_m where idNov=".$dat['id']);
                          $d->modGeneral("delete from n_nov_prog where id = ".$dat['id']);
                       }
                       $dat = $d->getGeneral1("select count(id) as num  
                                                from n_nov_prog where idEmp=".$idEmp);              
                       if ($dat['num']==0)
                           $id = $d->modGeneralId("insert into n_nov_prog (idEmp, fecha, idSup) 
                                               values(".$idEmp.", '".$datP['fecha']."', ".$data->idSup.")");

                       // Recorrer dias del turno para replicarlo
                       $ini = $data->diaInicio;
                       for($i=$ini;$i<=$data->diasMes;$i++)
                       {
                          # Se identifica el inicio y fin del inicio de los turnos  
                        //echo $data->idTur.' dia '.$data->diaIni.'<br />';
                          $datos = $d->getGeneral("select b.*, c.neutro as descanso   
                                                      from n_turnos_g a 
                                                        inner join n_turnos_g_h b on b.idTur = a.id 
                                                        inner join n_horarios c on c.id = b.idHor 
                                                      where a.id = ".$data->idTur." and b.orden >= ".$data->diaIni." order by b.orden  ");
                          foreach ($datos as $dat) 
                          { 
                             $idHor = $dat['idHor'];
                             $descanso = $dat['descanso'];
                             $cam = 't'.$i;  
                             $idMod = 1;
                             if ($i<=$diaPer)
                             {

                                $d->modGeneral("update n_nov_prog set $cam = ".$idHor." 
                                          where idEmp = ".$idEmp);  
                         ...
                         $festivo = $dat['festivo'];
                                ...
                                $this->getActualizar( $id, $idHor, $descanso, $i, $idEmp, $domingo, $festivo );
                ...
                }// Fin tipo 1 insertar datos de turnos 
                if ($data->tipo==2)// Editar horario
                {
                    $val = $data->idHor;
                    $cam = 't'.$data->diasMes;  
                    ...
                    //echo $cam.' - '.$val.' <br />'; 
                    $d->modGeneral("update n_nov_prog set $cam = ".$val." 
                                         where idEmp = ".$data->idEmp);    
                    $descanso = 0;

                    $dat = $d->getGeneral1("select  
                                                 case when DAYOFWEEK( concat( ano ,'-', lpad( mes ,2,'0' ), '-', 
                                             lpad( ".$data->diasMes." ,2,'0' ) ) ) = 1 then 1 else 0 end as domingo 
                                             from n_nov_prog_p");        
                    $domingo = $dat['domingo'];
                         // verificar festivo 
                         $dat = $d->getGeneral1("select count( a.id) as festivo  
                             from c_general_dnh a 
                                  inner join n_nov_prog_p b on b.ano = year( a.fecha ) and b.mes = month( a.fecha ) and day( a.fecha ) = ".$data->diasMes);
                         $festivo = $dat['festivo'];

                    $this->getActualizar( $datHor['id'] , $data->idHor, $descanso, $data->diasMes, $data->idEmp, $domingo, $festivo );

                }
                if ($data->tipo==3)// Registro de reemplazos
                {                

                    $datReg = $d->getGeneral1("select id 
                                        from n_nov_prog 
                                      where idEmp = ".$data->idEmp." 
                                         and idSup = ".$data->idSup." 
                                         and year(fecha) =".$ano." 
                                         and month(fecha) = ".$mes  );                    
                    $d->modGeneral("delete from n_nov_prog_t 
                                      where idEmp = ".$data->idEmp." 
                                         and idNov = ".$datReg['id'] );                    
                    // Guardar reeemplazo de empleado
                    $d->modGeneral("insert into n_nov_prog_t 
                                        (idEmp,idNov, idPues, idSup, dia, comen, idTnov) 
                  values(".$data->idEmp.",".$datReg['id'].",".$data->idPue.", ".$data->idSup.", ".$data->diasMes.",'".$data->comen."' ,".$data->idNov.")");                                          
                 }
                $connection->commit();
//                return $this->redirect()->toUrl($this->getRequest()->getBaseUrl().$this->lin.'i/'.$id);// El 1 es para mostrar mensaje de guardado
        ...
        $mes    = $datP['mesNumero'];     
        $dat = $d->getGeneral1("select day( last_day(
                                  concat( ".$datP['ano']." ,'-', lpad( ".$datP['mesNumero'].",2,'0' ), '-01'  )  ) ) as dia");
...
//echo 'd '.$ano.' - '.$diasMes;
        ...
        (
            "titulo"    => $this->tlis,
            "datos"     => $d->getTurnosPrograma($data->idSup, $data->idPue, $ano, $mes), 
...
//echo $dia." ".$data->idEmp;
    ...
    public function getActualizar( $id, $idHor, $descanso, $dia, $idEmp, $domingo, $festivo )
    {
        ...
        $datosE = $d->getGeneral1("select a.id, c.idPtra   
                                                from a_empleados a 
                                                    inner join n_proyectos_e b on b.idEmp = a.id and b.estado = 0
                                                    inner join n_proyectos_ep c on c.idIproy = b.id  
                                                   where a.id = ".$idEmp);  
       $idPuesto = $datosE['idPtra']; 
       // Validar variables en conceptos guardados en horarios 
...
//echo '--------'.$idHor.'<br />';
       ...
       {                                                   
           ...
           $d->modGeneral("insert into n_nov_prog_m ( idNov, idHor, idHfor, tipo, dia, idEmp, domingo, festivo, idPues ) 
              values (".$id.",".$idHor.",".$datH['id'].",".$datH['tipo'].",".$dia.", ".$idEmp.", ".$domingo.", ".$festivo.", ".$idPuesto.")"); 
       }
       // Validar permiso de trabajo 
       if ($descanso==1)
       {  
           $d->modGeneral("insert into n_nov_prog_m ( idNov, idHfor, tipo, dia, descanso, idEmp, domingo, festivo, idPues ) 
              values (".$id.",1 ,0,".$dia.", ".$descanso.", ".$idEmp.", ".$domingo.", ".$festivo.", ".$idPuesto.")");                                          
       }
    }                          
